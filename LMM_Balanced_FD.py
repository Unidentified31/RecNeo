...
# Randomly subsample Non-Neologisms to balance with Neologisms
neologisms = data[data["Role"] == "Neologism"]
non_neologisms = data[data["Role"] == "Non-Neologism"]
subsampled_non_neologisms = non_neologisms.sample(n=len(neologisms), random_state=None)  # Different random subset each time
balanced_data = pd.concat([neologisms, subsampled_non_neologisms])

# Fit a simplified Linear Mixed-Effects Model (LMM) for FD
model = smf.mixedlm(
    formula="FD ~ Role",  # Fixed effect: Role
    data=balanced_data,  # Use balanced data
    groups=balanced_data["Participant"],  # Random effect: Participant
